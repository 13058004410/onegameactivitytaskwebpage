                ...
                <div>
                  ...
                  <div class="content_wrapper" v-if="!loginValue">
                    ...
                    <div class="body" v-if="activeIndex === 0">
                      ...
                      <div>
                        <span @click="openModal(0)">
                          <img class="eachDayBetAmount" :src="require('@/assets/img/washCode_1/btn_h5.png')" alt="">
                        </span>
                      </div>
                    </div>
                    <!-- 今天----------------------------------------------------- -->
                ...
                <div class="box-content">
                  ...
                  <ul>
                    ...
                    <li>
                      <p>
                        2.
                        体育投注洗码有效投注额，统计周期为北京时间前一日的0:00至今日的0:00，ONE游戏各厅皆独立计算有效投注额；
                      </p>
                    </li>
                    <li>
                      <p>
                        3.系统自动添加日洗码时间为：当日12:00之前结算完毕，并添加昨日对应会员VIP等级洗码礼金到会员账户；
                      </p>
                    </li>
                    <li>
                      <p>
...
export default {
  ...
  data() {
    return {
      ...
      tabContent: [
        {
          yesToday: getYestodayDate(),  //昨天
          today: getTodayDate(),  //今天
        ...
        }, // 上周
        {
          yesToday: getYestodayDate(),  //昨天
          today: getTodayDate(),  //今天
          beginDate: getTodayDate() + ' 00:00:00',  
          endDate: getCurDate() + ' 23:59:59',
          betCount: "0.00",
          giftCount: "0.00",
          status: 1,
          xmList: [],
  ...
  computed: {
    ...
    historyList() {
      if (this.activeIndex === 0) {  //0是昨天 1是今天
  ...
  updated() {
    console.log(this.washCodeList[this.washCodeList.length-1],this.activeIndex,this.$store.state.appInfo)
  },
  mounted() {
    // setTimeout(() => {
  ...
  methods: {
    ...
    async loadTicket(data) {
      console.log('打印loadTicket1111111111')
      this.website = true
      if (data.loginStatus) {
        ...
        await this.getHistoryList(1)  // 查询洗码提案列表  今天
        await this.getHistoryList(2)  // 查询洗码提案列表  昨天
    ...
    loadWashCodeList() {
      ...
      if ( that.activeIndex===0 ) {
        that.getWashCodeList(that.activeIndex)
      ...
      await axios
        ...
        .then((res) => {
          ...
          if (flag === "0000") {
            ...
            that.getHistoryList(1) // 查询洗码提案（本周）今天
            that.getHistoryList(2) // 查询洗码提案（上周）昨天
            setTimeout(() => {
    ...
    openModal(index) {
      ...
      if(that.website && that.loginName) {
        console.log(that.website,that.loginName)
        that.getWashCodeList(type);
        that.showModal = true;
      }else {
        let h5Token = sessionStorage.getItem("token");
        if (h5Token) {
          console.log(h5Token,that.loginName)
          that.getWashCodeList(type);
          setTimeout(() => {
            that.showModal = true;
          }, 500);
  ...
  watch: {
    ...
    //   that.getHistoryList(1) // 查询洗码提案（本周）//今天
    //   that.getHistoryList(2) // 查询洗码提案（上周）//昨天
    //   console.log('监听h5TokenStore')
    //   this.loadWashCodeList()  //获取用户周洗码汇总统计列表
    ...
    h5TokenStore: {
      handler:function( newValue ){
        ...
        if( newValue.loginName && newValue.h5Token ){
          let that = this;
          that.$loading.hide();
          that.getHistoryList(1) // 查询洗码提案（本周）//今天
          that.getHistoryList(2) // 查询洗码提案（上周）//昨天
          that.getWashCodeInfo(); // 获取本周洗码数据
          that.loadWashCodeList()  //获取用户周洗码汇总统计列表
